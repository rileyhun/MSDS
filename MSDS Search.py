...
# read list of Analyzers to be searched
...
# Conduct MSDS Searches on SafeTec
    for i in range(len(Analyzers)):
        ...
        Classification_Type = wait.until(EC.visibility_of_element_located((By.ID, "placeBody_dynField60_dropClassType")))
        Select(Classification_Type).select_by_visible_text("Instrument/Analyzer")
...
def MSDS_SearchBySDSNumber(Numbers):

    # read list of MSDS Numbers to be searched
    SDS_data = pd.read_csv(Numbers, header=None)
    SDS_data.columns = ["SDS-Numbers"]
    Numbers = SDS_data["SDS-Numbers"]
...
# Conduct MSDS Searches on SafeTec
    for i in range(len(Numbers)):
        ...
        SDS_Number = wait.until(EC.visibility_of_element_located((By.ID, "placeBody_dynField3_txtTextBox")))
        SDS_Number.clear()
        SDS_Number.send_keys(Numbers[i])
        ...
        for record_page in range(0, number_pages):
            ...
            for j in range(0, len(list_links)):
                ...
                if i == 0 and record_page==0 and j==0:
                    data.to_csv("MSDS Inventory Results "+"{:%B %d, %Y}".format(datetime.now())+".csv", mode='a',
                                header=["SDS-Number", "Product Name", "Manufacturer", "SDS-Number",
                                                        "LL Number", "Chemical Component", "CAS Number", "% Context",
                                                        "Min %", "Max %", "Avg %", "% Range", "SARA 313 %"],index=False)
                else:
                    data.to_csv("MSDS Inventory Results "+"{:%B %d, %Y}".format(datetime.now())+".csv", mode='a',
...
# Conduct MSDS Searches on SafeTec
    for i in range(len(CAS)):
        ...
        Ingredient_CAS_Number = wait.until(EC.visibility_of_element_located((By.ID, "placeBody_dynField48_txtTextBox")))
        Ingredient_CAS_Number.clear()
        ...
        for record_page in range(0, number_pages):
            ...
            for j in range(0, len(list_links)):
                ...
                if i == 0 and record_page==0 and j==0:
                    data.to_csv("MSDS Inventory Results "+"{:%B %d, %Y}".format(datetime.now())+".csv", mode='a',
                                header=["CAS_Number", "Product Name", "Manufacturer", "SDS-Number",
                                                        "LL Number", "Chemical Component", "CAS Number", "% Context",
                                                        "Min %", "Max %", "Avg %", "% Range", "SARA 313 %"],index=False)
                else:
                    data.to_csv("MSDS Inventory Results "+"{:%B %d, %Y}".format(datetime.now())+".csv", mode='a',
